  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingCategory
        ? ('http://localhost:3000/api/v1/admin/products/' + editingCategory.id)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-5">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Gestión de productos, precios, control de existencias y disponibilidad en tienda.
          </p>
        </div>
          ...
          <div className="bg-background border border-border rounded-xl w-full max-w-md shadow-2xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <h3 className="font-bold text-base text-foreground">
                {editingCategory ? 'Editar Categoría' : 'Nuevo Producto'}
              </h3>
              <button
